...
/****** Object:  StoredProcedure [Control].[Latest]    Script Date: 06/02/2023 15:05:26 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [Control].[Latest]
(
...
BEGIN TRY
	...
	SELECT	'EXEC Tag.Latest @TableFrom='''+@DB+'.'+QUOTENAME(s.name)+'.'+QUOTENAME(o.name)+''', @TableTo='''+@DB+'.'+QUOTENAME(st.name)+'.'+QUOTENAME(ot.name)+''', @DateFrom=@DateFrom OUTPUT, @DateTo=@DateTo OUTPUT, @DryRun=@DryRun' SQL, 
	...
	FROM	[WH_Silver].sys.schemas s 
			...
			INNER JOIN [WH_Silver].sys.objects ot ON ot.schema_id = st.schema_id AND ot.name = o.name+'_Latest' AND ot.type = 'U'

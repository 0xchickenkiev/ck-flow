...
/****** Object:  StoredProcedure [Control].[First]    Script Date: 06/02/2023 15:04:49 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [Control].[First]
(
...
BEGIN TRY
	...
	SELECT	'EXEC Tag.First @TableFrom='''+@DB+'.'+QUOTENAME(s.name)+'.'+QUOTENAME(o.name)+''', @TableTo='''+@DB+'.'+QUOTENAME(st.name)+'.'+QUOTENAME(ot.name)+''', @DateFrom=@DateFrom OUTPUT, @DateTo=@DateTo OUTPUT, @DryRun=@DryRun' SQL, 
	...
	FROM	[WH_Silver].sys.schemas s 
			...
			INNER JOIN [WH_Silver].sys.objects ot ON ot.schema_id = st.schema_id AND ot.name = o.name+'_First' AND ot.type = 'U'

...
/****** Object:  StoredProcedure [Utility].[Partitioning]    Script Date: 06/02/2023 15:06:56 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [Utility].[Partitioning]
(	
...
AS
	...
	DECLARE	@Compression TABLE (i int IDENTITY(1,1),SchemaName sysname, ObjectName sysname, IndexName sysname NULL, PartitionNumber int, TargetCompression varchar(60))

	DECLARE	@SchemeCount int,
			@i int,
			@j int

	DECLARE @PrimaryFilegroup bit
...
BEGIN
	...
	SET @Boundary = ISNULL(@Boundary, DATEADD(DAY,1,CONVERT(DATE, GETDATE())))
	...
	SET @SQL = 'SELECT @PrimaryFilegroup = CASE COUNT(*) WHEN 0 THEN 1 ELSE 0 END FROM sys.filegroups WHERE Name LIKE '''+ISNULL(@Database,db_name())+'\_'+@Function+'\_20[0-9][0-9][0-1][0-9]'' ESCAPE ''\'''
	EXEC dbo.sp_executesql @SQL, N'@PrimaryFilegroup bit OUTPUT', @PrimaryFilegroup=@PrimaryFilegroup OUTPUT
	...
	SET @SQL = 'SELECT * FROM '+Utility.RemoteQuery(@SQL,@Server,DEFAULT)+' x'
	...
	BEGIN
		...
		IF (SELECT Action FROM  @Boundaries WHERE i = @i) = 'Insert'
			BEGIN
				...
				SELECT	@SQL += 'ALTER PARTITION SCHEME '+ s.Scheme +' NEXT USED '+CASE WHEN Boundary = '9999-12-31 23:59:59.999' OR @PrimaryFilegroup = 1 THEN REPLACE(ISNULL(@Database,db_name()), 'WH_', '') +'_Default' ELSE REPLACE(ISNULL(@Database,db_name()), 'WH_', '') +'_'+@Function+'_'+CONVERT(char(6),REPLACE(b.Boundary,'-','')) END+'; ' 
...
--select @SQL
	...
	BEGIN
		...
		SET @SQL = 'SELECT * FROM '+Utility.RemoteQuery(@SQL,@Server,DEFAULT)+' x ORDER BY SchemaName DESC, ObjectName DESC, PartitionNumber DESC, IndexName DESC'
--select @SQL
